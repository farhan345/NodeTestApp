...
exports.storeRegister = asyncErrorCatch(async (req, res, next) => {
    ...
    if (oldstore) {
        ...
        return next(new ErrorHandler(400, "Store Already Registered On This Email"))
    }
    ...
    const blockStore = await storeModel.findOne({ email: email, isStoreBlock: true });
    if (blockStore) {
        return next(new ErrorHandler(400, "Sorry you are not be able to register using this email"))
    }
...
exports.storeLogin = asyncErrorCatch(async (req, res, next) => {
    ...
    const msg = "OTP is send on your email, please verify your email before login"
    const email = req.body.email.toLowerCase();
    ...
    if (store.isStoreBlock === true) {
        return next(new ErrorHandler(400, "sorry, your account has been blocked by admin"))
    }
    ...
    res.status(200).json({ success: true, message: "Login Successfully", token, store })
})
...
exports.forgotPassword = asyncErrorCatch(async (req, res, next) => {
    ...
    if (obj) {
        ...
        try {
            ...
            const htmlTemplate = fs.readFileSync('./template/04-forgot-password.html', 'utf8');
            ...
            res.status(200).json({ success: true, message: "OTP for password recovery is send on your email", store })
        } catch (error) {
...
const sendOTPForEmailVerification = async ({ _id, email }, res, msg, token, isSend) => {
    let store;
    try {
        ...
        const htmlTemplate = fs.readFileSync('./template/02-email-verification.html', 'utf8');
        const compiledTemplate = handlebars.compile(htmlTemplate);
        const htmlModified = compiledTemplate({
...
exports.verifyStoreEmailVerificationOTP = asyncErrorCatch(async (req, res, next) => {
    if (!req.body.OTP) {
    ...
    const emailverifyOTP = crypto.createHash('sha256').update(req.body.OTP).digest('hex');
    const store = await storeModel.findOne({ emailverifyOTP, _id: req.body._id, emailVerifyOTPExpire: { $gt: Date.now() } });
    if (!store) {
    ...
    res.status(200).json({ success: true, message: "Email verified", store: store })
})
...
exports.verifyResetPasswordOTP = asyncErrorCatch(async (req, res, next) => {
    ...
    const resetPasswordOTP = crypto.createHash('sha256').update(req.body.OTP).digest('hex');
    const store = await storeModel.findOne({ resetPasswordOTP, _id: req.body._id, resetPasswordExpire: { $gt: Date.now() } });
    if (!store) {
        return next(new ErrorHandler(400, "Your OTP was expired"))
    }
    res.status(200).json({ success: true, message: "OTP verified", store: store })
})
...
exports.getstoreOwnProfile = asyncErrorCatch(async (req, res, next) => {
    ...
    return res.status(200).json({ success: true, store, message: "Store detail fetched successfully" })
})
...
exports.updateStoreOwnProfile = asyncErrorCatch(async (req, res, next) => {
    ...
    // console.log(newData);
    storeModel.findByIdAndUpdate(req.body._id, newData, { runValidators: true }, async (err, result) => {
        if (err) {
            if (req?.files?.storeProfile) {
                fs.unlinkSync(req.files.storeProfile[0].path);
            }
            ...
            const store = await storeModel.findById(req.body._id);
            res.status(200).json({ success: true, msg: "store profile updated", store })
        }
    });
...
exports.getAllstoresByAdmin = asyncErrorCatch(async (req, res, next) => {
    const stores = await storeModel.find().sort({ createdAt: -1 }).select("+isEmailVerified").select("+isDocumentVerified");
    if (!stores) {
        return next(new ErrorHandler(400, "No store Found"));
    }
    res.status(200).json({ success: true, stores, message: "All stores fetched successfully" })
})

exports.getAllstoresByAdminWhoAreNotVerified = asyncErrorCatch(async (req, res, next) => {
    ...
    const stores = await storeModel.find(query, { emailVerifyOTPExpire: 0, emailverifyOTP: 0, reviews: 0 }).sort({ createdAt: -1 }).select("+isEmailVerified").select("+isDocumentVerified").populate({ path: "storeCategoryType", select: "storeCategoryType" });
    if (!stores) {
        return next(new ErrorHandler(400, "No store Found"));
    }
    res.status(200).json({ success: true, stores, message: "All stores fetched successfully" })
...
exports.getAllstoresByAdminWhoAreVerified = asyncErrorCatch(async (req, res, next) => {
    ...
    const stores = await storeModel.find(query, { emailVerifyOTPExpire: 0, emailverifyOTP: 0, reviews: 0 }).sort({ verifiedAt: -1 }).select("+isEmailVerified").select("+isDocumentVerified").populate({ path: "storeCategoryType", select: "storeCategoryType" });
    if (!stores) {
        return next(new ErrorHandler(400, "No store Found"));
    }
    res.status(200).json({ success: true, stores, message: "All stores fetched successfully" })
...
exports.updatestoreStatusByAdmin = asyncErrorCatch(async (req, res, next) => {
    ...
    const Data = {
        ...
        isStoreBlock: req.body.status === "blocked" ? true : (req.body.status === "active" ? false : false),
        // accessLevel: req.body.accessLevel || (req.body?.isDocumentVerified && req.body?.status !== "blocked") ? 0 : 1,
        updatedAt: month1,

    }
    if (req.body.status === "verified" || req.body.status === "rejected" || req.body.status === "pending") {
        console.log("inside");
    ...
    storeModel.findByIdAndUpdate(req.params.id, newData, (err, result) => {
        if (err) {
            ...
            notificationMsg = ""
            if (req.body.status === "blocked") {
                notificationMsg = "We hope this message finds you well. We wanted to inform you that your account on Skip A Line has been temporarily blocked by our administration team. This action has been taken due to a violation of our platform's terms of use or community guidelines."
            } else if (req.body.status === "active") {
                notificationMsg = "We're pleased to inform you that your account on Skip A Line has been successfully reactivated by our administration team. We apologize for any inconvenience the temporary suspension may have caused."
            } else if (req.body.status === "verified") {
                notificationMsg = "We're pleased to inform you that your submitted documents have been successfully verified by our administration team. Your account on Skip A Line is now fully verified, and you can access all features and benefits available to verified users."
            } else if (req.body.status === "rejected") {
                notificationMsg = "We regret to inform you that your submitted documents have not been approved during the verification process. Our administration team found discrepancies or issues with the provided information, leading to the rejection of your verification."
            }

            const htmlTemplate = fs.readFileSync('./template/03-unsubscribe.html', 'utf8');
            const compiledTemplate = handlebars.compile(htmlTemplate);
            const htmlModified = compiledTemplate({
...
exports.getSinglestoreDetail = asyncErrorCatch(async (req, res, next) => {
    ...
    if (!store) {
        ...
        const recentOrders = await orderModel
            .find({ store: req.params.id }).populate("user orderStatus")
            .sort({ createdAt: -1 })
            .limit(20)
            .lean();
        ...
        const orders = await orderModel.find({ store: req.params.id }).populate({ path: "orderStatus" });
        if (!orders) {
            return next(new ErrorHandler(400, "No order Found"))
        }

        let totalPrice = 0, totalOrders = 0, completedOrders = 0, inProgress = 0, paid = 0, ready = 0;
        ...
        res.status(200).json({ success: true, store, totalEarning: totalPrice.toFixed(2), totalOrders, completedOrders, inProgress, paid, ready })
        // res
        //     .status(200)
...
exports.getSinglestoreDetailByUser = asyncErrorCatch(async (req, res, next) => {
    ...
    res.status(200).json({ success: true, store, message: "Store detail fetched successfully" })
})
exports.getStoreDocument = asyncErrorCatch(async (req, res, next) => {

    const store = await storeModel.findOne({ _id: req.params.id }, { document: 1 });
    if (!store) {
        return next(new ErrorHandler(404, "No store Found"))
    }
...
exports.getTotalNumberOfStoresNotVerified = asyncErrorCatch(async (req, res, next) => {
    const totalStoresNotVerified = await storeModel.find({ isDocumentVerified: false }).count();
    //console.log(totalStoresNotVerified);
    res.status(200).json({ success: true, totalStoresNotVerified })
});


exports.getTotalNumberofVerifiedStores = asyncErrorCatch(async (req, res, next) => {
    const verifiedStores = await storeModel.find({ isDocumentVerified: true }).count();
    res.status(200).json({ success: true, verifiedStores });
});


exports.getTotalNumberOfStoresBlock = asyncErrorCatch(async (req, res, next) => {
    ...
    const statistics = await storeModel.aggregate([
        {
            $group: {
                _id: null,
                totalStores: { $sum: 1 },
                totalStoresNotVerified: { $sum: { $cond: [{ $eq: ["$isDocumentVerified", false] }, 1, 0] } },
                verifiedStores: { $sum: { $cond: [{ $eq: ["$isDocumentVerified", true] }, 1, 0] } },
                blockedStores: { $sum: { $cond: [{ $eq: ["$status", "blocked"] }, 1, 0] } }
            }
        }
    ]);


    const { totalStores, totalStoresNotVerified, verifiedStores, blockedStores } = statistics[0];
...
exports.getTotalNumberOfStoresByMonth = asyncErrorCatch(async (req, res, next) => {
    ...
    let toatalBlockedStores = [0, 0, 0, 0]
    ...
    let toatalVerifiedStores = [0, 0, 0, 0]
    ...
    res.status(200).json({ success: true, toatalVerifiedStores, toatalBlockedStores, lastFourMonths })
    // res.status(200).json({ success: true, toatalBlockedStores, lastFourMonths })
});
// combined monthly blocked and verified data ends

exports.getTotalNumberOfStoresBlockByMonth = asyncErrorCatch(async (req, res, next) => {
...
exports.getTotalNumberOfstoresByAdminWhoAreVerified = asyncErrorCatch(async (req, res, next) => {
    debugger;
    // get the current date
    const currentDate = new Date();
    ...
    const stores = await storeModel.find({ isDocumentVerified: true, isEmailVerified: true })
    if (!stores) {
        return next(new ErrorHandler(404, "No store Found"));
    }
    ...
    res.status(200).json({ success: true, toatalVerifiedStores, lastFourMonths })
