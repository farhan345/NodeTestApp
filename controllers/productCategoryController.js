  ...
  productCategoryModel.create(Data, async (err, result) => {
    if (err) {
      ...
      res
        ...
        .json({
          success: true,
          message: "product category addedd successfully",
        });
    }
  });
});
...
exports.deleteProductCategory = asyncErrorCatch(async (req, res, next) => {
  ...
  if (!productCategory) {
    return next(new ErrorHandler(404, "Product category not found"));
  }
  ...
  res
    ...
    .json({
      success: true,
      message: "Product category and associated products removed",
    });
});
...
exports.updateProductCategoryr = asyncErrorCatch(async (req, res, next) => {
  ...
  const newData = await isObjectPropertyEmpty(
    productCategoryData,
    productCategory
  );
