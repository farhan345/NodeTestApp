...
exports.addProductInOffer = asyncErrorCatch(async (req, res, next) => {
    ...
    for (let j = 0; j < req.body?.offerProducts?.length; j++) {
        if (!req.body.offerProducts[j].product) {
            return next(new ErrorHandler(400, `Please enter your${req.body?.offerProducts?.length > 1 ? `${j + 1}` : ""} offer item product Id`))
        }
        else if (!req.body.offerProducts[j].stock) {
            return next(new ErrorHandler(400, `Please enter your${req.body?.offerProducts?.length > 1 ? `${j + 1}` : ""} offer item product quantity`))
        }
    }
    for (let prod of req.body.offerProducts) {
        const existingOfferProduct = await offerModel.findOne({ store: req.body.store, "offerProducts.product": prod?.product });
        if (existingOfferProduct !== null) {
            if (existingOfferProduct?.offerCategory !== req.body.offerCategory) {
                ...
                return next(new ErrorHandler(400, `You cant add this product ${productold?.productName} as it is already added in some other offer category`))
            }
        }
    }
    const existingOffer = await offerModel.findOne({ store: req.body.store, offerCategory: req.body.offerCategory })

    const offerProducts = req.body.offerProducts
    let updatedField = 0, updateStock = 0, checkProduct = 0;
    let existingProductsArray = [];
    const productsID = req.body.offerProducts.map((obj) => { return obj })
    ...
    const isStoreAndCategoryExist = await offerModel.find({ store: req.body.store, offerCategory: req.body.offerCategory });
    if (isStoreAndCategoryExist.length === 0) {
        for (let obj of req.body.offerProducts) {
            ...
            if (product) {
                if (product.quantity < obj.stock) {
                    return next(new ErrorHandler(400, `Sorry, you can't add this product (${product.productName}) in offer becuase it is out of stock`))
                } else {
                    checkProduct++;
                }
            }
            ...
            offerModel.create(Data, async (err, result) => {
                if (err) {
                    ...
                    for (let i = 0; i < req.body.offerProducts.length; i++) {
                        const product = await productModel.findById(offerProducts[i].product);
                        if (product) {
                            ...
                            product.save({ validateBeforeSave: false }, (err, result) => {
                                ...
                                if (updateStock === req.body.offerProducts.length) {
                                    storeModel.updateOne({ _id: req.body.store }, { isStoreHasOffer: true, offerPercentage: discountPercentage }, (err, result) => {
                                        if (err) {
                                            return next(new ErrorHandler(400, err.message))
                                        } else {
                                            res.status(200).json({ success: true, message: "Products added in offer" })
                                        }
                                    })
        ...
        for (let offer of productsID) {
            ...
            const product = await offerModel.findOne({ store: req.body.store, offerCategory: req.body.offerCategory, offerProducts: { $elemMatch: { product: offer.product } } }, { offerProducts: { $elemMatch: { product: offer.product } } })
            if (product) {

                const existingProduct = await productModel.findOne({ _id: offer.product });
                if (existingProduct) {

                    if (existingProduct.quantity < offer.stock) {
                        return next(new ErrorHandler(400, `Sorry, you can't add this product (${existingProduct.productName}) in offer becuase it is out of stock`))
                    } else {
                        // console.log(product.offerProducts[0].stock);
                        // res.status(200).json({product})

                        let newStock = product.offerProducts[0].stock + offer.stock;
                        offerModel.updateOne({ store: req.body.store, offerCategory: req.body.offerCategory, "offerProducts.product": offer.product }, { $set: { "offerProducts.$.stock": newStock } }, (err, result) => {
                            if (err) {
                                return next(new ErrorHandler(400, err.message))
                            } else {
                                console.log("percentage2", discountPercentage);
                                console.log("product2", existingProduct);
                                const discountedPrice = existingProduct.price * (1 - discountPercentage / 100);
                                const roundedPrice = Math.round(discountedPrice);
                                ...
                                existingProduct.save({ validateBeforeSave: false }, async (err, result) => {
                                    if (err) {
                                        return next(new ErrorHandler(400, err.message))
                                    } else {
                                        updatedField++
                                        if (updatedField === productsID.length) {
                                            storeModel.updateOne({ _id: req.body.store }, { isStoreHasOffer: true, offerPercentage: discountPercentage }, (err, result) => {
                                                if (err) {
                                                    ...
                                                    res.status(200).json({ success: true, message: "All products are added in this offer" })
            ...
            if (!product) {
                ...
                if (oldProduct) {
                    if (oldProduct.quantity < offer.stock) {
                        return next(new ErrorHandler(400, `Sorry, you can't add this product (${oldProduct.productName}) in offer becuase it is out of stock`))
                    } else {
                        offerModel.updateOne({ store: req.body.store, offerCategory: req.body.offerCategory }, { $push: { offerProducts: offer } }, (err, result) => {
                            if (err) {
                                return next(new ErrorHandler(400, err.message))
                            } else {
                                console.log("percentage3", discountPercentage);
                                console.log("product3", oldProduct);
                                const discountedPrice = oldProduct.price * (1 - discountPercentage / 100);
                                const roundedPrice = Math.round(discountedPrice);
                                ...
                                oldProduct.save({ validateBeforeSave: false }, async (err, result) => {
                                    if (err) {
                                        return next(new ErrorHandler(400, err.message))
                                    } else {
                                        updatedField++
...
exports.createOfferCategoryType = asyncErrorCatch(async (req, res, next) => {
    ...
    const Data = {
        ...
        dateTillPromoAvailable: formattedDate,
    }
    ...
    if (existingOfferCategory) {
        ...
        offerModel.create(Data, async (err, result) => {
            ...
            res.status(200).json({ success: true, message: "Offer category type created" })
        })
    }
...
exports.updateOfferCategoryType = asyncErrorCatch(async (req, res, next) => {
    ...
    offerModel.updateOne({ store: req.body.store, offerCategory: req.body.offerCategory }, newData, { runValidators: true }, async (err, result) => {
        if (err) {
            return next(new ErrorHandler(400, err.message))
        }
        else {
...
exports.removeProductFromOffer = asyncErrorCatch(async (req, res, next) => {
    ...
    if (req.body?.offerProducts?.length === 0) {
        return next(new ErrorHandler(400, "Please select product that you want to be remove from offer"))
    }
    const deletedProducts = req.body.offerProducts.map((obj) => { return obj.product })

    const beforeUpdatestore = await offerModel.findOne({ store: req.body.store, offerCategory: req.body.offerCategory })
    ...
    if (beforeUpdatestore?.offerProducts?.length !== 0 && deletedProducts?.length !== 0) {
        console.log("check 1");
        for (let i = 0; i < deletedProducts?.length; i++) {

            const offerProduct = await offerModel.findOne({ store: req.body.store, offerCategory: req.body.offerCategory, "offerProducts.product": deletedProducts[i] })
            console.log(offerProduct, "lok");
            if (!offerProduct) {
                return next(new ErrorHandler(404, "product not found in that offer category type"))
            }
        ...
        offerModel.updateOne({ store: req.body.store, offerCategory: req.body.offerCategory }, { $pull: { offerProducts: { product: { $in: deletedProducts } } } }, async (err, result) => {
            if (err) {
                return next(new ErrorHandler(400, "Products not found"))
            } else {
                console.log("check2");
                const afterUpdatestore = await offerModel.findOne({ store: req.body.store, offerCategory: req.body.offerCategory })
                if (!afterUpdatestore) {
                    return next(new ErrorHandler(404, "Offer Not Found"))
                } else {
                    console.log("check3");
                    if (afterUpdatestore?.offerProducts?.length === 0) {
                        ...
                        const allStores = await offerModel.find({ store: req?.body?.store })
                        if (allStores.length === 0) {
                            ...
                            if (allStores?.length === findStoreLength) {
                                console.log("check6");
                                storeModel.updateOne({ _id: req.body.store }, { isStoreHasOffer: false, offerPercentage: 0 }, (err, result) => {
                                    if (err) {
                                        return next(new ErrorHandler(400, err.message))
                                    } else {
                                        console.log("check7");
                                        for (let i = 0; i < deletedProducts?.length; i++) {
                                            for (let j = 0; j < beforeUpdatestore?.offerProducts?.length; j++) {
                                                console.log(deletedProducts[i], beforeUpdatestore?.offerProducts[j]?.product);
                                                if (mongoose.Types.ObjectId(deletedProducts[i]).equals(beforeUpdatestore?.offerProducts[j]?.product._id)) {
                                                    console.log("check8");
                                                    if (beforeUpdatestore?.offerProducts[j]?.stock !== 0) {
                                                        console.log("check9");
                                                        productModel.updateOne({ _id: beforeUpdatestore?.offerProducts[j]?.product._id }, { $inc: { quantity: beforeUpdatestore?.offerProducts[j]?.stock }, $set: { isAvailableInOffer: false, discountedPrice: 0, offPercentage: 0, dateTillAvailableInOffer: null } }, (err, result) => {
                                                            if (err) {
                                                                return next(new ErrorHandler(400, err.message))
                                                            } else {
                                                                res.status(200).json({ success: true, message: "product removed from offer" })
...
exports.removeCompleteOffer = asyncErrorCatch(async (req, res, next) => {
    ...
    const offer = await offerModel.findOne({ store: req.body.store, offerCategory: req.body.offerCategory });
    if (!offer) {
        return next(new ErrorHandler(404, "offer not found"))
    }
    console.log(offer);
    ...
    if (offer?.offerProducts?.length !== 0) {
        for (let j = 0; j < offer?.offerProducts?.length; j++) {
            if (offer?.offerProducts[j]?.stock !== 0) {
                productModel.updateOne({ _id: offer?.offerProducts[j]?.product }, { $inc: { quantity: offer?.offerProducts[j]?.stock }, $set: { isAvailableInOffer: false, discountedPrice: 0, offPercentage: 0, dateTillAvailableInOffer: null } }, async (err, result) => {
                    console.log("update");
                    if (err) {
                        return next(new ErrorHandler(400, err.message))
                    } else {
                        ...
                        if (count === offer?.offerProducts?.length) {
                            const allStoreOffers = await offerModel.find({ store: req.body.store });
                            if (allStoreOffers?.length === 0) {
                                storeModel.updateOne({ _id: req.body.store }, { isStoreHasOffer: false, offerPercentage: 0 }, (err, result) => {
                                    if (err) {
                                        return next(new ErrorHandler(400, err.message))
                                    }
                                    offerModel.deleteOne({ store: req.body.store, offerCategory: req.body.offerCategory }, (err, result) => {
                                        if (err) {
                                            return next(new ErrorHandler(400, err.message))
                                        } else {
                                            res.status(200).json({ success: true, message: "product removed from offer" })
                                ...
                                offerModel.deleteOne({ store: req.body.store, offerCategory: req.body.offerCategory }, (err, result) => {
                                    if (err) {
                                        ...
                                        res.status(200).json({ success: true, message: "Offer Category removed" })
                                    }
                                })
                            }
...
exports.getAllOfferProducts = asyncErrorCatch(async (req, res, next) => {
    ...
    const offerProducts = await offerModel.find({ store: req.body.store, offerCategory: req.body.offerCategory }, { offerProducts: 1 })
    ...
    if (offerProducts.length !== 0) {
        const productInfo = await productModel.find({
            ...
            _id: { $in: offerProducts[0].offerProducts.map((obj) => { return obj.product }) }
        }, { productName: 1, price: 1, quantity: 1, image: 1, _id: 1, discountedPrice: 1, isAvailableInOffer: 1, store: 1, category: 1 }).populate({ path: "category", select: "categoryName" });
        if (productInfo.length !== 0) {
            res.status(200).json({ success: true, offerProducts: productInfo, message: "All offer products retrieved successfully" })
        }
        if (productInfo?.length === 0) {
            return next(new ErrorHandler(404, "No Product Found"))
        }
...
console.log(offerProducts,"offerProducts");
    ...
    const filter={
        ...
        _id: { $in: offerProducts.flatMap((obj) => obj.offerProducts.map((obj) => obj.product)) }
    }
    if(query?.categoryId){
    ...
    if (offerProducts.length !== 0) {
        const productInfo = await productModel.find(filter, {
            productName: 1,
            price: 1,
            image: 1,
            _id: 1,
        ...
        if (productInfo.length !== 0) {
            res.status(200).json({ success: true, products: updatedProducts, message: "All offer products retrieved successfully" })
        }
        if (productInfo?.length === 0) {
            return next(new ErrorHandler(404, "No Product Found"))
        }
...
exports.getAllStoresThatHaveOffer = asyncErrorCatch(async (req, res, next) => {
    ...
    const stores = await offerModel.find({ dateTillPromoAvailable: { $gte: new Date() }, isDeleted: false }, { store: 1 }).populate("store")
    if (stores?.length == 0) {
    ...
    res.status(200).json({ success: true, message: "offer stores fetched successfully", offerStores: storesWithDistance })
})
exports.getAllStoresThatHaveOffer = asyncErrorCatch(async (req, res, next) => {
    if (!req.body.userLatitude) {
        return next(new ErrorHandler(400, "please enter user latitude"))
    ...
    const { userLatitude, userLongitude } = req.body
    const today = new Date();
    const storeIds = await offerModel.distinct("store", { dateTillPromoAvailable: { $gte: new Date() }, isDeleted: false });
...
exports.getAllOfferCategories = asyncErrorCatch(async (req, res, next) => {
    ...
    const offersCategories = await offerModel.find({ store: req.body.store }, { offerCategory: 1, discountedPercentage: 1, dateTillPromoAvailable: 1 })
    if (offersCategories?.length === 0) {
        return next(new ErrorHandler(404, "No offers found of this store"))
    }
    res.status(200).json({ success: true, message: "Offer categories fetched successfully", offersCategories })
})

exports.getAllProductsOFSpecificOfferCategory = asyncErrorCatch(async (req, res, next) => {
    if (!req.body.store) {
        return next(new ErrorHandler(400, "please enter store Id"))
    }
    if (!req.body.offerCategory) {
    ...
    const filteredProducts = offer?.offerProducts.filter((offerProduct) => {
        ...
        return product.productName.toLowerCase().includes(req.body.searchText.toLowerCase())
            || product.productBarcode.toLowerCase().includes(req.body.searchText.toLowerCase());
    });
    ...
    // }
    res.status(200).json({ success: true, message: "offer category products retrieved successfully", offerCategoryProducts: filteredOffer })
...
exports.removeOfferAutomaticallyIfExpire = asyncErrorCatch(async (req, res, next) => {
