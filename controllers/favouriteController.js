...
exports.addToFavourite = asyncErrorCatch(async (req, res, next) => {
    ...
    const favourite = await favouriteModel.findOne({ user: req.body.user, store: req.body.store })
    if (favourite) {
        favouriteModel.deleteOne({ user: req.body.user, store: req.body.store }, (err, result) => {
            if (err) {
                return next(new ErrorHandler(400, err.message))
            }
            storeModel.updateOne(
                { _id: req.body.store },
                { $set: { isStoreInFavourite: false } }, (err, result1) => {
                    if (err) {
                        return next(new ErrorHandler(400, err.message))
                    }
                    res.status(201).json({ success: true, message: "Store removed from favourite" })
                }
            );
        })
        ...
        favouriteModel.create(Data, (err, result) => {
            ...
            storeModel.updateOne(
                { _id: req.body.store },
                { $set: { isStoreInFavourite: true } }, (err, result1) => {
                    if (err) {
                        return next(new ErrorHandler(400, err.message))
                    }
                    res.status(201).json({ success: true, message: "Store Addedd in your favourite" })
                }
            );
...
exports.getAllFavouriteStores = asyncErrorCatch(async (req, res, next) => {
    ...
    const favourite = await favouriteModel.find({ user: req.body.user }).populate({ path: "store", select: "profile name ratings address location isStoreInFavourite" })
...
console.log(storeLat,storeLng);
    ...
    res.status(200).json({ success: true, favouriteStores: storesWithDistance, message: "All favourite stores retrieved successfully" })
})
