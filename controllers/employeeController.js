...
const store=await storeModel.findOne({_id:req.body.store,isStoreBlock:false},{name:1});
if(!store){
...
const email = `${formattedemployeeName}-${Math.floor(Math.random() * 10000)}@${formattedDomainName}.com`;
//const email = `${formattedemployeeName}-${Math.floor(Math.random() * 10000)}@${formattedStoreName}.com`;
// const sanitizedEmail = email.replace(/[^a-zA-Z0-9@.-]/g, '_');
    ...
    employeeModel.create(Data, async (err, result) => {
        if (err) {
            ...
            res.status(200).json({success:true,message:"Employee registered successfully",result})
        }
    });
})
...
exports.addForgotPasswordEmail=asyncErrorCatch(async(req,res,next)=>{
    ...
    const existingEmail=await employeeModel.find({ emailForForgotPassword: forgotEmail, _id: { $ne: new ObjectId(req.body._id) } })
...
const sendOTPForEmailVerification = async ({ _id, emailForForgotPassword }, res, msg, token) => {
    let employee;
    try {
...
exports.employeeLogin = asyncErrorCatch(async (req, res, next) => {
    ...
    if(employee.status==="inactive"){
        return next(new ErrorHandler(401, "You are not an active employee, please update your status"));
    }
    ...
    if(employee?.emailForForgotPassword===""){
        res.status(400).json({success:false,token:"",employee,message:"Forgot Password Email Not Found"})
    }
    if(!employee?.isEmailForgotPasswordVerified){
        res.status(400).json({success:false,token:"",employee,message:"Forgot password email not verified please register it again"})
    }
    const token=await employee.getJWTToken();
    res.status(200).json({success:true,token,employee,message:"Login Successfully"})
})
...
exports.forgotPassword = asyncErrorCatch(async (req, res, next) => {
    ...
    const employee = await employeeModel.findOne({ emailForForgotPassword: req.body.emailForForgotPassword });
    if (!employee) {
        return next(new ErrorHandler(400, "You are not able to forgot your password, please contact from store."))
    }
    if(employee.status==="inactive"){
        return next(new ErrorHandler(401, "you are not an active employee, please update your status"));
    }
    ...
    try {
        sendEmailToemployee({
            ...
            subject: "E-Store",
            message,
        }).then((data) => { res.status(200).json({ success: true, message: "OTP for password recovery is send on your email", employee }) }).
            catch((error) => {
                res.status(400).json({ success: false, message: "please click on resend it again" })
            })
    } catch (error) {
...
exports.verifyResetPasswordOTP = asyncErrorCatch(async (req, res, next) => {
    ...
    const employee = await employeeModel.findOne({ resetPasswordOTP,_id:req.body._id, resetPasswordExpire: { $gt: Date.now() } });
    if (!employee) {
        return next(new ErrorHandler(400, "Your OTP was expired"))
    }
    res.status(200).json({ success: true, message: "OTP verified", employee: employee })
})

exports.verifyEmployeeEmailVerificationOTP = asyncErrorCatch(async (req, res, next) => {
    if (!req.body.OTP) {
    ...
    const employee = await employeeModel.findOne({ emailverifyOTP, _id: req.body._id, emailVerifyOTPExpire: { $gt: Date.now() } });
    if (!employee) {
        return next(new ErrorHandler(400, "Your OTP was expired"))
    }
    ...
    res.status(200).json({ success: true, message: "Email verified", employee: employee })
})
...
exports.resetPassword = asyncErrorCatch(async (req, res, next) => {
    ...
    res.status(200).json({success:true,message:"Password updated",employee})
...
exports.updateEmployeeOwnPassword = asyncErrorCatch(async (req, res, next) => {
    ...
    const employee = await employeeModel.findById(req.body._id).select("+password");
...
exports.getEmployeeOwnProfile = asyncErrorCatch(async (req, res, next) => {
    ...
    return res.status(200).json({ success: true, employee,message:"Employee detail fetched successfully" })
})
// store owners
exports.getAllEmployeesByStoreOwner = asyncErrorCatch(async (req, res, next) => {
    if(!req.body.store){
        return next(new ErrorHandler(400,"Please provide store Id"))
    }
    const employees = await employeeModel.find({store:req.body.store,isDeleted:false}).sort({createdAt:-1});
    if (!employees) {
        return next(new ErrorHandler(404, "No employee Found"));
    }
    res.status(200).json({ success: true, employees,message:"All employees fetched successfully" })
})

exports.updateEmployeeInfoByStoreOwner=asyncErrorCatch(async(req,res,next)=>{
    if(!req.body.name){
        return next(new ErrorHandler(400,"Please enter employee name"))
    }
    if(!req.body.password){
    ...
    const employee=await employeeModel.findOne({_id:req.body._id,store:req.body.store,isDeleted:false});
    if(!employee){
        return next(new ErrorHandler(400,"Employee not found"))
    }
    const Data = {
    ...
    employeeModel.findByIdAndUpdate(req.body._id,newData,(err,result)=>{
        if(err){
            ...
            return res.status(200).json({ success: true, msg: "Employee details updated",employee:result})
        }
    })
...
exports.getSingleEmployeeDetail = asyncErrorCatch(async (req, res, next) => {
    ...
    res.status(200).json({ success: true, employee,message:"Employee detail fetched successfully" })
})
