...
exports.userRegister = asyncErrorCatch(async (req, res, next) => {
    ...
    userModel.create(Data, async (err, result) => {
        if (err) {
            fs.unlinkSync(req.file.path);
...
exports.registerUserDocument = asyncErrorCatch(async (req, res, next) => {
    if (!req?.file?.path) {
        return next(new ErrorHandler(400, "Please select file that you want to be verified"))
    }
    ...
    userModel.findByIdAndUpdate(req.body._id, newData, { runValidators: true }, async (err, result) => {
        if (err) {
            fs.unlinkSync(req?.file?.path);
            return next(err)
        }
            ...
            let notificationMsg = "Thank you for submitting your documents for verification on Skip A Line. Our administration team will now review the documents you have submitted. Please allow some time for the review to be completed."
            ...
            return res.status(200).json({ success: true, message: "Document submit for verification" })
        }
    })
...
exports.forgotPassword = asyncErrorCatch(async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: "OTP for password recovery is send on your email", user })
    } catch (error) {
...
exports.verifyEmailVerificationOTP = asyncErrorCatch(async (req, res, next) => {
    ...
    const user = await userModel.findOne({ emailverifyOTP, _id: req.body._id, emailVerifyOTPExpire: { $gt: Date.now() } });
    if (!user) {
    ...
    res.status(200).json({ success: true, message: "Email verified", user: user })
})
...
exports.verifyResetPasswordOTP = asyncErrorCatch(async (req, res, next) => {
    ...
    if (!req.body.OTP) {
        return next(new ErrorHandler(400, "Please enter forgot password recovery OTP"))
    }
    const resetPasswordOTP = crypto.createHash('sha256').update(req.body.OTP).digest('hex');
    const user = await userModel.findOne({ resetPasswordOTP, _id: req.body._id, resetPasswordExpire: { $gt: Date.now() } });
    if (!user) {
        return next(new ErrorHandler(400, "Your OTP was expired"))
    }
...
exports.getAllusersByAdmin = asyncErrorCatch(async (req, res, next) => {
    const users = await userModel.find().select("+isEmailVerified").select("+isDocumentVerified");
    if (!users) {
...
exports.getAllusersByAdminWhoAreNotVerified = asyncErrorCatch(async (req, res, next) => {
    ...
    const users = await userModel.find(query).sort({ createdAt: -1 }).select("+isEmailVerified").select("+isDocumentVerified");
    if (users?.length === 0) {
        return next(new ErrorHandler(400, "No user Found"));
    }
...
exports.getAllusersByAdminWhoAreVerified = asyncErrorCatch(async (req, res, next) => {
    ...
    const users = await userModel.find(query).select("+isEmailVerified").sort({ createdAt: -1 }).select("+isDocumentVerified");
    if (!users) {
        return next(new ErrorHandler(400, "No user Found"));
    }
    res.status(200).json({ success: true, users })
...
exports.updateUserStatusByAdmin = asyncErrorCatch(async (req, res, next) => {
    ...
    const Data = {
        ...
        isUserBlock: req.body.status === "blocked" ? true : (req.body.status === "active" ? false : false),
...
exports.getSingleuserDetail = asyncErrorCatch(async (req, res, next) => {
    ...
    const user = await userModel.findById(req.params.id).select("+isEmailVerified").select("+isDocumentVerified");
    if (!user) {
        return next(new ErrorHandler(400, "No user Found"))
    }
...
exports.getTotalNumberOfUsersNotVerified = asyncErrorCatch(async (req, res, next) => {
    const totalUsersNotVerified = await userModel.find({ isDocumentVerified: false }).count();
    console.log(totalUsersNotVerified);
    res.status(200).json({ success: true, totalUsersNotVerified })
});

exports.getTotalNumberOfUsersVerified = asyncErrorCatch(async (req, res, next) => {
    const verifiedUsers = await userModel.find({ isDocumentVerified: true }).count();
    //console.log(totalUsersNotVerified);
    ...
    const statistics = await userModel.aggregate([
        {
            $group: {
                _id: null,
                totalUsers: { $sum: 1 },
                totalUsersNotVerified: { $sum: { $cond: [{ $eq: ["$isDocumentVerified", false] }, 1, 0] } },
                verifiedUsers: { $sum: { $cond: [{ $eq: ["$isDocumentVerified", true] }, 1, 0] } },
                blockedUsers: { $sum: { $cond: [{ $eq: ["$isUserBlock", true] }, 1, 0] } }
            }
        }
    ]);
    const { totalUsers, totalUsersNotVerified, verifiedUsers, blockedUsers } = statistics[0];
...
exports.getTotalNumberOfUsersBlocked = asyncErrorCatch(async (req, res, next) => {
...
exports.getTotalNumberOfUsersBlockByMonth = asyncErrorCatch(async (req, res, next) => {
...
exports.getTotalNumberOfUsersByAdminWhoAreVerified = asyncErrorCatch(async (req, res, next) => {
    // get the current date
    const currentDate = new Date();
    ...
    const users = await userModel.find({ isDocumentVerified: true, isEmailVerified: true })
    ...
    res.status(200).json({ success: true, toatalVerifiedUsers, lastFourMonths })
})
...
exports.getSingleUserDetailHistory = asyncErrorCatch(async (req, res, next) => {
    ...
    if (!user) {
        ...
        const recentOrders = await orderModel
            .find({ user: req.params.id }).populate("store orderStatus")
            .sort({ createdAt: -1 })
            .limit(20)
            .lean();
        ...
        const orders = await orderModel.find({ user: req.params.id }).populate({ path: "orderStatus" });
        if (!orders) {
        ...
        res.status(200).json({ success: true, user, totalEarning: totalPrice.toFixed(2), totalOrders, completedOrders, inProgress, paid, ready })
        // res
        //     .status(200)
...
exports.getTotalNumberOfUsersByMonth = asyncErrorCatch(async (req, res, next) => {
    const currentDate = new Date();
    const lastFourMonths = [];
    for (let i = 3; i >= 0; i--) {
        const monthDate = new Date(currentDate.getFullYear(), currentDate.getMonth() - i, 1);
    ...
    let toatalVerifiedUsers = [0, 0, 0, 0];
    const verifiedUsers = await userModel.find({ isDocumentVerified: true, isEmailVerified: true })
    ...
    res.status(200).json({ success: true, toatalBlockedUsers, toatalVerifiedUsers, lastFourMonths });
});
