...
exports.newOrder = asyncErrorCatch(async (req, res, next) => {
    ...
    if (!req.body.platformFee) {
        return next(new ErrorHandler(400, "Please enter order items shipping price"))
    }
    if (!req.body.totalPrice) {
        return next(new ErrorHandler(400, "Please enter order items total price"))
    }
    if (!req.body.user) {
    ...
    for (let j = 0; j < req.body?.orderItems?.length; j++) {
        if (!req.body.orderItems[j].productName) {
            return next(new ErrorHandler(400, `Please enter your${req.body.orderItems.length > 1 ? `${j + 1}` : ""} order item product name`))
        } else if (!req.body.orderItems[j].price) {
            return next(new ErrorHandler(400, `Please enter your${req.body.orderItems.length > 1 ? `${j + 1}` : ""} order item product price`))
        }
    ...
    if (isOnlineOrder) {
        const userCart = await cartModel.findOne({ user: user, store: store, status: "active" });
        if (!userCart) {
            return next(new ErrorHandler(404, "User cart not found"))
        } else {
            cartModel.findOneAndDelete({ user: user, store: store, status: "active" }, (err, result) => {
                if (err) {
                    ...
                    orderModel.create({
                        ...
                        user,
                        orderType: isOnlineOrder ? "Promo Deals" : "Store Picks",
                        //  orderType: isOnlineOrder ? "Promo Deals" : "Store Picks",
                        ...
                        if (result) {
                            ...
                            const orderList=await orderModel.findById(result?._id).populate({
                                path: "orderItems.product", populate: {
                                    path: "category",
            ...
            if you have not requested this email then please ignore it`;
                            ...
                            const htmlTemplate = fs.readFileSync('./template/13-order-placed.html', 'utf8');
                            ...
                            const htmlModified = compiledTemplate({
                                ...
                                total: parseFloat((result?.itemsPrice + result.platformFee).toFixed(2))
    ...
    if (!isOnlineOrder) {
        for (let i = 0; i < orderItems?.length; i++) {
            ...
            const orderProduct = await productModel.findOne({ _id: orderItems[i]?.product });
                ...
                orderProduct.save({ validateBeforeSave: false }, (err, result) => {
                    ...
                    if you have not requested this email then please ignore it`;
                                    ...
                                    const htmlModified = compiledTemplate({
                                        ...
                                        subtotal:result?.itemsPrice,
                                        platformFee:result?.platformFee,
                                        total: parseFloat((result?.itemsPrice + 10.99).toFixed(2))
...
exports.getAllUserOrdersByStoreOwner = asyncErrorCatch(async (req, res, next) => {
    if (!req.body.store) {
        return next(new ErrorHandler(400, "Please enter store Id"))
    }
    // if (!req.body.user) {
    //     return next(new ErrorHandler(400, "Please enter User Id"))
    // }
    // const order = await orderModel.find({ store: req.body.store,user:req.body.user }, { _id: 1, user: 1, orderStatus: 1, itemsPrice: 1, shippingPrice: 1 ,orderType:1,totalPrice:1}).populate({path:"user",select:"name email _id profile number"}).populate({path:"orderStatus"});


    const { orderType, orderStatusId } = req.body;
...
exports.getSingleOrderDetailByUser = asyncErrorCatch(async (req, res, next) => {
    ...
    const orders = await orderModel.findOne({ _id: req.body._id, store: req.body.store, user: req.body.user }, { _id: 1, store: 1, orderStatus: 1, itemsPrice: 1, platformFee: 1, orderItems: 1, isOnlineOrder: 1, orderType: 1 }).populate("orderStatus").populate({
        path: "orderItems.product", populate: {
            path: "category",
...
exports.getSingleOrderDetailByStoreOwner = asyncErrorCatch(async (req, res, next) => {
    // if (!req.body.user) {
    //     return next(new ErrorHandler(400, "Please enter user Id"))
    // }
    // if (!req.body.store) {
    ...
    const order = await orderModel.findOne({ _id: req.body._id }, { _id: 1, user: 1, orderStatus: 1, itemsPrice: 1, platformFee: 1, totalPrice: 1, orderItems: 1 }).populate(
        "user",
        "name email _id profile number"
    ).populate("orderStatus").populate({ path: "orderItems.product", select: "isAvailableInOffer image" });
    console.log(order);
    if (!order) {
        return next(new ErrorHandler(404, "Orders not found "));
    }
...
exports.getSingleOrderDetailByStoreOwnerUsi = asyncErrorCatch(async (req, res, next) => {
    if (!req.body.user) {
        return next(new ErrorHandler(400, "Please enter user Id"))
    }
    if (!req.body.store) {
    ...
    const order = await orderModel.findOne({ _id: req.body._id, user: req.body.user, store: req.body.store }, { _id: 1, user: 1, orderStatus: 1, itemsPrice: 1, platformFee: 1, totalPrice: 1, orderItems: 1 }).populate(
        "user",
        "name email _id profile number"
    ).populate("orderStatus").populate({ path: "orderItems.product", select: "isAvailableInOffer image" });
    console.log(order);
...
exports.getUSerOwnOrders = asyncErrorCatch(async (req, res, next) => {
    ...
    const order = await orderModel
        .find(filters, { _id: 1, store: 1, orderStatus: 1, itemsPrice: 1, platformFee: 1, isOnlineOrder: 1, orderType: 1, orderItems: 1 }).sort({ createdAt: -1 })
    ...
    // const order = await orderModel
    // .find({ user: req.body.user }, { _id: 1, store: 1, orderStatus: 1, itemsPrice: 1, shippingPrice: 1,isOnlineOrder:1,orderType:1 })
    // .populate("store orderStatus")
...
exports.updateUserOrderStatus = asyncErrorCatch(async (req, res, next) => {
    ...
    const orderList=await orderModel.findById(req.body._id ).populate({
        path: "orderItems.product", populate: {
            path: "category",
...
if you have not requested this email then please ignore it`;
    ...
    const compiledTemplate = handlebars.compile(htmlTemplate);
    let orderMsg="";
    if(req.body.orderStatusId==="64394488439d40de5a6f1243"){
orderMsg="ORDER IS IN PROGRESS"
    }else if(req.body.orderStatusId==="64394474439d40de5a6f1240"){
        orderMsg="ORDER IS READY"
    }else if(req.body.orderStatusId==="641d64fad225a66ae3f9a848"){
...
exports.getAllOrdersOfSingleStore = asyncErrorCatch(async (req, res, next) => {
    ...
    const ordersQuery = orderModel.find(query).sort({ createdAt: -1 }).populate("user orderStatus");
...
exports.getAllOrdersOfSingleUser = asyncErrorCatch(async (req, res, next) => {
    ...
    const ordersQuery = orderModel.find(query).sort({ createdAt: -1 }).populate("user store orderStatus");
    const countQuery = orderModel.countDocuments(query);
...
exports.getSingleStoreOrderDetail = asyncErrorCatch(async (req, res, next) => {
    const order = await orderModel.findOne({ _id: req.params.id }).populate("user store orderStatus").populate({ path: "orderItems.product", populate: { path: "category" } })
...
//         return next(new ErrorHandler(400,"Please enter store Id"))
//     }
//     const orders=await orderModel.find({store:req.body.store,orderType:"Store Picks"}).populate({path:"orderStatus"});
//     if(!orders){
...
//     for (let order of orders) {
//         if (order?.orderType === "Store Picks") {
...
exports.getTotalEarningsInstore = asyncErrorCatch(async (req, res, next) => {
    ...
    const startDate = moment(req.body.startDate).set({ hour: 0, minute: 0, second: 0 });
    const endDate = moment(req.body.endDate).set({ hour: 23, minute: 59, second: 59 });
    ...
    const existingDatesSet = new Set(orders.map(order => moment(order.createdAt).startOf('day').format('YYYY-MM-DD')));
    ...
    while (currentDate.isSameOrBefore(endDate)) {
        ...
        if (existingDate) {
            ...
            const order = orders.find(order => moment(order.createdAt).startOf('day').format('YYYY-MM-DD') === currentDateFormatted);
            if (order?.orderType === "Store Picks") {
                storeEarning += order.totalPrice;
                storeOrders += 1;
                if (order?.orderStatus?.statusType === "Completed") {
...
//     const query = { store: req.body.store, orderType: "Store Picks" };
//     if (startDate) {
//       query.createdAt = { $gte: startDate };
//     }
//     const orders = await orderModel.find(query).populate({ path: "orderStatus" });
